...
function buildTradeContext(
  ...
  const processScores = closed.filter((t) => t.process_score != null).map((t) => Number(t.process_score));
...
## Emotion Breakdown
${Object.entries(emotionCounts).map(([e, d]) => `- ${e}: ${d.count} trades, total P&L $${d.pnl.toFixed(2)}`).join("\n")}
...
`;
  ...
  for (const t of recent) {
    ...
    const date = t.close_timestamp ? String(t.close_timestamp).split("T")[0] : String(t.open_timestamp).split("T")[0];
