...
import { Ghost, Plus, ArrowUpRight, ArrowDownRight, Clock, CheckCircle2, Target, ShieldAlert } from "lucide-react";
...
export default function PhantomTradesPage() {
  ...
  const active = phantoms.filter((p) => p.status === "active");
  const resolved = phantoms.filter((p) => p.status === "resolved");
  ...
  function getDirectionIndicator(p: PhantomTrade) {
    // If we have cached price data, show whether price is moving toward target or stop
  ...
  function outcomeLabel(outcome: string | null) {
    switch (outcome) {
      case "target_hit": return { text: "Target Hit", color: "text-green-400 bg-green-400/10" };
      case "stop_hit": return { text: "Stop Hit", color: "text-red-400 bg-red-400/10" };
      case "partial": return { text: "Partial", color: "text-yellow-400 bg-yellow-400/10" };
      case "neither": return { text: "Neither", color: "text-muted-foreground bg-muted/50" };
      default: return { text: "Active", color: "text-blue-400 bg-blue-400/10" };
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Phantom Trades</h1>
            <p className="text-sm text-muted-foreground">Trades you considered but passed on</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            {tab === "active" ? "No active phantom trades. Log one to start tracking missed opportunities." : "No resolved phantom trades yet."}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {displayed.map((p) => {
            const badge = outcomeLabel(p.outcome);
            ...
            return (
              <Link
                ...
                className="block rounded-xl bg-surface border border-border p-4 hover:border-accent/30 transition-colors"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 text-right">
                    ...
                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${badge.color}`}>
                      {p.status === "resolved" && p.outcome === "target_hit" && <CheckCircle2 size={12} />}
                      {badge.text}
                    </span>
                  </div>
                </div>
