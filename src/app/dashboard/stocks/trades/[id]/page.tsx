...
export default function StockTradeDetailPage() {
  ...
  const fetchTrade = useCallback(async () => {
    const { data: allTrades } = await supabase
      .from("stock_trades").select("id, open_timestamp").order("open_timestamp", { ascending: false });
    const ids = (allTrades ?? []).map((t: { id: string }) => t.id);

    if (ids.length > 0) {
      setAllTradeIds(ids);
      const { data } = await supabase.from("stock_trades").select("*").eq("id", tradeId).single();
      if (data) { setTrade(data as StockTrade); setLoading(false); return; }
    }

    setAllTradeIds(DEMO_STOCK_TRADES.map((t) => t.id));
    const demo = DEMO_STOCK_TRADES.find((t) => t.id === tradeId);
    setTrade(demo ?? null);
    setLoading(false);
  }, [supabase, tradeId]);
  ...
  if (!trade) return (
    <div className="text-center py-20">
      ...
      <button onClick={() => router.push("/dashboard/stocks/trades")} className="text-accent hover:underline mt-2 text-sm">Back to trades</button>
    </div>
  );
  ...
  const duration = trade.close_timestamp
    ...
    : "Open";
  const costBasis = trade.asset_type === "option" && trade.premium_per_contract && trade.contracts
    ? trade.premium_per_contract * trade.contracts * 100
    ...
    <div className="space-y-6 mx-auto max-w-[1200px] pb-20">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              {trade.sector && (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent font-semibold">{trade.sector}</span>
              )}
            </div>
            <p className="text-xs text-muted mt-0.5">
              {trade.company_name && `${trade.company_name} · `}{new Date(trade.open_timestamp).toLocaleString()}
              {trade.market_session && ` · ${trade.market_session.replace("_", " ")}`}
            </p>
          </div>
        </div>
        <div className="text-right">
          ...
          {!isOpen && costBasis > 0 && <p className="text-xs text-muted">{((pnl / costBasis) * 100).toFixed(2)}% return</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="glass rounded-2xl border border-border/50 p-5 space-y-4" style={{ boxShadow: "var(--shadow-card)" }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Stat label={isOption ? "Contracts" : "Quantity"} value={isOption ? (trade.contracts?.toString() ?? "—") : trade.quantity.toString()} />
          ...
          {isOption && (
            <div className="pt-2 border-t border-border/30">
              <p className="text-[10px] uppercase tracking-wider text-muted font-semibold mb-2">Option Details</p>
              <div className="grid grid-cols-2 gap-3">
                <Stat label="Strike" value={trade.strike_price ? `$${trade.strike_price}` : "—"} />
                <Stat label="Expiry" value={trade.expiration_date ? new Date(trade.expiration_date).toLocaleDateString() : "—"} />
                <Stat label="Premium" value={trade.premium_per_contract ? `$${trade.premium_per_contract.toFixed(2)}` : "—"} />
                <Stat label="Underlying" value={trade.underlying_symbol ?? "—"} />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass rounded-2xl border border-border/50 p-5 space-y-3" style={{ boxShadow: "var(--shadow-card)" }}>
          <h3 className="text-xs font-semibold text-muted uppercase tracking-wider flex items-center gap-1.5">
            <FileText size={14} className="text-accent" /> Post-Trade Review
          </h3>
          {trade.review ? (
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border/30">
        <button onClick={() => prevId && router.push(`/dashboard/stocks/trades/${prevId}`)} disabled={!prevId}
        ...
        <button onClick={() => nextId && router.push(`/dashboard/stocks/trades/${nextId}`)} disabled={!nextId}
          className="flex items-center gap-1.5 text-sm text-muted hover:text-foreground disabled:opacity-30 transition-colors">
