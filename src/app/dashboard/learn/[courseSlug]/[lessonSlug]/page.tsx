...
export default function LessonViewerPage({
  ...
  const canUseNova = hasAccess("interactive-lessons");
  ...
  // Pro/Max users: interactive Nova-guided lessons
  if (canUseNova) {
    return (
      <LessonChat course={course} lesson={lesson} lessonIndex={lessonIndex} />
    );
  }

  // Free users: static lesson content
      ...
      <div className="border-t border-border/50 pt-6">
        {result ? (
          <div className="glass rounded-xl p-6 border border-profit/20 bg-profit/5 text-center mb-6">
            <CheckCircle2 size={32} className="text-profit mx-auto mb-3" />
            <h3 className="text-base font-bold text-profit mb-1">
              {result.courseComplete ? "Course Complete!" : "Lesson Complete!"}
            </h3>
