...
export default function AchievementsPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="animate-pulse text-accent">Loading achievements...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="glass rounded-2xl border border-border/50 p-5 mb-6" style={{ boxShadow: "var(--shadow-card)" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((achievement) => {
          ...
          const progressValue = getProgressValue(achievement.id, progress);
          ...
          if (achievement.tiers) {
            ...
            for (const t of tiers) {
              if (
                unlocked.some(
                  (u) =>
                    u.achievement_id === achievement.id && u.tier === t
                )
              ) {
                highestTier = t;
                break;
          ...
          if (achievement.tiers) {
            ...
            if (nextThreshold === 0 && achievement.tiers.length > 0) {
              // All tiers unlocked
              nextThreshold =
                achievement.tiers[achievement.tiers.length - 1].threshold;
              currentTierThreshold = nextThreshold;
            }
          } else {
            // Single tier
            nextThreshold =
              achievement.metric === "total_trades"
                ? 1
                : achievement.metric === "total_trades_100"
                  ? 100
                  : achievement.metric === "total_trades_1000"
                    ? 1000
                    : achievement.metric === "account_age_365"
                      ? 365
                      : achievement.metric === "journal_entries_500"
                        ? 500
                        : 1;
          }
          ...
          const tierMeta = highestTier ? TIER_META[highestTier] : null;
          const isActive =
            ...
            >
              ...
              <div className="mt-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {unlockStatus && progressValue >= nextThreshold && (
