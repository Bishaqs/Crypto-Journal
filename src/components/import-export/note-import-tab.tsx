...
export function NoteImportTab() {
  ...
  async function startImport() {
    ...
    for (let i = 0; i < chunks.length; i++) {
      const { error } = await supabase.from("journal_notes").insert(chunks[i]);
      if (error) {
    ...
    <div className="space-y-5">
      <div className="glass border border-border/50 rounded-2xl p-5">
        ...
        <p className="text-xs text-muted mb-4">
          Import notes from a CSV file. Required column: <span className="text-accent font-medium">content</span>. Optional: title, tags, date, type.
        </p>
        ...
        {step === "upload" && (
          <div className="space-y-4">
            ...
            <div className="text-xs text-muted px-3 py-2 rounded-lg bg-surface border border-border">
              ...
              <code className="text-accent text-[10px]">title, content, tags, date, type</code>
              <p className="mt-1">Tags can be comma or semicolon-separated. Date accepts most formats (ISO, MM/DD/YYYY, etc).</p>
            </div>
        ...
        {step === "preview" && result && (
          <div className="space-y-4">
            ...
            {/* Column mapping */}
            <div>
              <p className="text-[11px] text-muted uppercase tracking-wider font-semibold mb-2">Column Mapping</p>
            ...
            {/* Preview table */}
            <div>
              <p className="text-[11px] text-muted uppercase tracking-wider font-semibold mb-2">Preview (first 5)</p>
              <div className="overflow-x-auto rounded-xl border border-border">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-surface border-b border-border">
                      <th className="px-3 py-2 text-left text-muted font-semibold">#</th>
                      <th className="px-3 py-2 text-left text-muted font-semibold">Title</th>
                      <th className="px-3 py-2 text-left text-muted font-semibold">Content</th>
                      <th className="px-3 py-2 text-left text-muted font-semibold">Tags</th>
                      <th className="px-3 py-2 text-left text-muted font-semibold">Date</th>
                  ...
                  <tbody>
                    {result.validRows.slice(0, 5).map((row) => (
                      <tr key={row.rowNumber} className="border-b border-border/30">
                        ...
                        <td className="px-3 py-2 text-foreground font-medium">{row.data.title || "Untitled"}</td>
                        <td className="px-3 py-2 text-foreground max-w-[200px]">{truncate(row.data.content || "", 80)}</td>
                        <td className="px-3 py-2 text-foreground">{row.data.tags || "—"}</td>
                        <td className="px-3 py-2 text-foreground whitespace-nowrap">{row.data.note_date || "Now"}</td>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              >
                Import {result.validRows.length} Note{result.validRows.length !== 1 ? "s" : ""}
              </button>
              <button
                onClick={reset}
        ...
        {step === "importing" && (
          <div className="text-center py-10">
            ...
            <p className="text-sm font-medium text-foreground mb-2">Importing notes...</p>
