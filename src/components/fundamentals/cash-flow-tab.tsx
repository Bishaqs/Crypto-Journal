...
import type { CashFlowData } from "./fundamentals-types";
import { CASH_FLOW_ROWS } from "./fundamentals-types";
...
const tooltipStyle = (colors: CashFlowTabProps["colors"]) => ({
...
export default function CashFlowTab({ data, symbol, colors }: CashFlowTabProps) {
  const { annual } = data;
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={annual}>
              ...
              <Line type="monotone" dataKey="operatingCashFlow" name="Operating" stroke={colors.win} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="investingCashFlow" name="Investing" stroke={colors.loss} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="financingCashFlow" name="Financing" stroke={colors.accent} strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={annual}>
              ...
              <Tooltip contentStyle={tooltipStyle(colors)} formatter={(value) => [formatCurrencyLarge(Number(value ?? 0)), ""]} />
              <Legend wrapperStyle={{ fontSize: "11px" }} />
              <Line type="monotone" dataKey="netIncome" name="Net Income" stroke={colors.win} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="capitalExpenditures" name="CapEx" stroke={colors.loss} strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      >
        <div className="p-5 pb-3">
          <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">
            {symbol} Cash Flow Statement (Annual)
          </h3>
        </div>
