...
import type { BalanceSheetData } from "./fundamentals-types";
import { BALANCE_SHEET_ROWS } from "./fundamentals-types";
...
const tooltipStyle = (colors: BalanceSheetTabProps["colors"]) => ({
...
export default function BalanceSheetTab({ data, symbol, colors }: BalanceSheetTabProps) {
  const { annual } = data;
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={annual}>
              ...
              <Line type="monotone" dataKey="totalAssets" name="Total Assets" stroke={colors.accent} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="totalLiabilities" name="Total Liabilities" stroke={colors.loss} strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={annual}>
              ...
              <Tooltip contentStyle={tooltipStyle(colors)} formatter={(value) => [formatCurrencyLarge(Number(value ?? 0)), ""]} />
              <Legend wrapperStyle={{ fontSize: "11px" }} />
              <Line type="monotone" dataKey="shareholderEquity" name="Shareholder Equity" stroke={colors.win} strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="longTermDebt" name="Long-Term Debt" stroke={colors.loss} strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {BALANCE_SHEET_ROWS.map((def, idx) => (
                <tr
                  key={def.key}
